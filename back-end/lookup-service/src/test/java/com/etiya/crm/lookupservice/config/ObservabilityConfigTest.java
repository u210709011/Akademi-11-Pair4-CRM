...
class ObservabilityConfigTest {
	...
	void keepsBusinessPaths_onServerSide() {
		assertThat(predicate.test("http.server.requests", serverContext("/api/v1/characteristics/1"))).isTrue();
	}

	@Test
	void excludesEurekaPaths_onClientSide() {
		assertThat(predicate.test("http.client.requests", clientContext("/eureka/apps/lookup-service"))).isFalse();
	}

	@Test
	void keepsOtherClientCalls_onClientSide() {
