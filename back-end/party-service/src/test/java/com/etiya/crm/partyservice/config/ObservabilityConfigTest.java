...
class ObservabilityConfigTest {
	...
	void keepsBusinessPaths_onServerSide() {
		assertThat(predicate.test("http.server.requests", serverContext("/api/v1/individuals/100"))).isTrue();
	}

	@Test
	void excludesEurekaPaths_onClientSide() {
		assertThat(predicate.test("http.client.requests", clientContext("/eureka/apps/party-service"))).isFalse();
	}

	@Test
	void keepsFeignCalls_onClientSide() {
		assertThat(predicate.test("http.client.requests", clientContext("/api/v1/general-types/resolve/x/y")))
				.isTrue();
	}
