...
class GlobalExceptionHandlerTest {
	...
	void handleNotFound_returns404() {
		...
		ResponseEntity<ErrorResponse> response = handler.handleNotFound(new OrderNotFoundException(1L), request);
	...
	@Test
	void handleMisconfiguration_returns500() {
		when(messageSource.getMessage(anyString(), any(), any(Locale.class))).thenReturn("Yapilandirma hatasi");

		ResponseEntity<ErrorResponse> response = handler.handleMisconfiguration(
				new BsnInterSpecNotFoundException("ORDER"), request);

		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.INTERNAL_SERVER_ERROR);
	}

	@Test
	void handleBadRequest_returns400_forGenericBusinessException() {
		when(messageSource.getMessage(anyString(), any(), any(Locale.class))).thenReturn("Gecersiz istek");

		ResponseEntity<ErrorResponse> response = handler.handleBadRequest(
				new CharacteristicValueMissingException(1L), request);

		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
	}

	@Test
	void handleOrderNotEditable_returns409() {
		when(messageSource.getMessage(anyString(), any(), any(Locale.class))).thenReturn("Duzenlenemez");

		ResponseEntity<ErrorResponse> response = handler.handleOrderNotEditable(
				new OrderNotEditableException(1L), request);
	...
	void handleValidationException_returns400WithFieldErrorsMap() {
		...
		when(bindingResult.getFieldErrors())
				.thenReturn(List.of(new FieldError("obj", "custId", "zorunlu alan")));
		...
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
		assertThat(response.getBody().validationErrors()).containsEntry("custId", "zorunlu alan");
	}
