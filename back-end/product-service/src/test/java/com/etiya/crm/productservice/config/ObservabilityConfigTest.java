...
class ObservabilityConfigTest {
	...
	void keepsBusinessPaths_onServerSide() {
		assertThat(predicate.test("http.server.requests", serverContext("/api/v1/products/1"))).isTrue();
	}

	@Test
	void excludesEurekaPaths_onClientSide() {
		assertThat(predicate.test("http.client.requests", clientContext("/eureka/apps/product-service"))).isFalse();
	}
