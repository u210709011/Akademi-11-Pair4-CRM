...
class GlobalExceptionHandlerTest {
	...
	void handleNotFound_returns404() {
		...
		ResponseEntity<ErrorResponse> response = handler.handleNotFound(new ProductNotFoundException(1L), request);
	...
	void handleConflict_returns409() {
		...
		ResponseEntity<ErrorResponse> response = handler.handleConflict(
				new ProductOfferingCharUseDuplicateException(1L, 2L), request);
