...
/** CUSER/UUSER alanlarini dolduran AuditorAware'in uc davranisini dogrular. */
class JwtAuditorAwareTest {
	...
	void returnsPreferredUsername_whenAuthenticatedWithJwt() {
		Jwt jwt = Jwt.withTokenValue("token")
				.header("alg", "none")
				.claim("preferred_username", "ahmet.yilmaz")
				.build();
	...
	void returnsSystem_whenPrincipalIsNotJwt() {
		TestingAuthenticationToken token = new TestingAuthenticationToken(Map.of("user", "x"), null, "ROLE_CRM_AGENT");
		token.setAuthenticated(true);
		SecurityContextHolder.getContext().setAuthentication(token);
