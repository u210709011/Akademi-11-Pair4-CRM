...
class ObservabilityConfigTest {
	...
	void keepsBusinessPaths_onServerSide() {
		assertThat(predicate.test("http.server.requests", serverContext("/api/v1/addresses/1"))).isTrue();
	}

	@Test
	void excludesEurekaPaths_onClientSide() {
		assertThat(predicate.test("http.client.requests", clientContext("/eureka/apps/contact-info-service")))
				.isFalse();
	}
